public class LeasingCost {
    ...
    String nameString = (description.substring(description.indexOf(("name:"))));
     String name = nameString.substring((nameString.indexOf('n')) + 5, (nameString.indexOf(';')));

     String dueString = (description.substring(description.indexOf(("due:"))));
     String due = dueString.substring((dueString.indexOf('d')) + 4, (dueString.indexOf(';')));

     String typeString = (description.substring(description.indexOf(("type:"))));
     String type = typeString.substring((typeString.indexOf('t')) + 5, (typeString.indexOf(';')));

     String lengthString = (description.substring(description.indexOf(("length:"))));
     String length = lengthString.substring((lengthString.indexOf('l')) + 7, (lengthString.indexOf(';')));

     String monthlyString = (description.substring(description.indexOf(("monthly:"))));
     String monthly = monthlyString.substring((monthlyString.indexOf('m')) + 8, (monthlyString.indexOf(';')));

      String mileUnitString = (description.substring(description.indexOf(("mile/unit:"))));
     String mileUnit = mileUnitString.substring((mileUnitString.indexOf('m')) + 10, (mileUnitString.indexOf(';')));

      String allowanceString = (description.substring(description.indexOf(("allowance:"))));
     String allowance = allowanceString.substring((allowanceString.indexOf('a')) + 10, (allowanceString.indexOf(';')));


     Lease LeaseFill = new Lease(Double.parseDouble(due), Integer.parseInt(length), Double.parseDouble(monthly), Integer.parseInt(allowance));

    if (description.contains("charger:") == true){
        String chargerString = (description.substring(description.indexOf(("charger:"))));
        String charger = chargerString.substring((chargerString.indexOf('c')) + 8, (chargerString.indexOf(';')));
        Fuel FuelElectric = new Fuel(Double.parseDouble(mileUnit), Double.parseDouble(charger));
        Vehicle VehicleElectric = new Vehicle(name, FuelElectric, LeaseFill);
        return VehicleElectric;
    }
    else{
        Fuel FuelGas = new Fuel(Double.parseDouble(mileUnit));
        Vehicle VehicleGas = new Vehicle(name, FuelGas, LeaseFill);
        return VehicleGas;
    }        
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        // COMPLETE THIS METHOD
            ...
            for (Vehicle lastMethod : vehicles){

                double due = lastMethod.getLease().getDueAtSigning();
                int length = lastMethod.getLease().getLeaseLength();
                double monthly = lastMethod.getLease().getMonthlyCost();
                double allowance = lastMethod.getLease().getMileageAllowance();

                double electricCombustion = lastMethod.getFuel().CO2EMITTED_ELECTRICITYCOMBUSTION;
                double gasCombustion = lastMethod.getFuel().CO2EMITTED_GASCOMBUSTION;
                double mileUnit = lastMethod.getFuel().getUsage();
                double charger = lastMethod.getFuel().getCharger();

                if (lastMethod.getFuel().getType() == 1){
                    lastMethod.setCO2Emission(computeCO2(length, mileUnit, allowance, gasCombustion));
                    lastMethod.setFuelCost(computeFuelCost(length, mileUnit, allowance, gasPrice));
                    lastMethod.setTotalCost((computeLeaseCost(due, length, monthly)) + (computeFuelCost(length, mileUnit, allowance, gasPrice)));
                }
                else{
                    lastMethod.setCO2Emission(computeCO2(length, mileUnit, allowance, electricCombustion));
                    lastMethod.setFuelCost(computeFuelCost(length, mileUnit, allowance, electricityPrice));
                    lastMethod.setTotalCost((computeLeaseCost(due, length, monthly)) + charger + (computeFuelCost(length, mileUnit, allowance, electricityPrice)));
                }
